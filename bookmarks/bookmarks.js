import {html, Polymer, Base, dom, templatize, afterNextRender, flush, useShadow, Templatizer, OptionalMutableDataBehavior, animationFrame, microTask, idlePeriod, Debouncer as Debouncer$1, enqueueDebouncer, matches, translate, calculateSplices} from "chrome://resources/polymer/v3_0/polymer/polymer_bundled.min.js";
import {loadTimeData} from "chrome://resources/js/load_time_data.m.js";
import {addWebUIListener, removeWebUIListener, isIOS, isWindows, isMac, sendWithPromise, addSingletonGetter, isAndroid} from "chrome://resources/js/cr.m.js";
...
const parseHtmlSubset = function() {
    ...
    const allowedAttributes = new Map([["href", (node,value)=>node.tagName === "A" && (value.startsWith("chrome://") || value.startsWith("https://"))], ["target", (node,value)=>node.tagName === "A" && value === "_blank"]]);
    const allowedOptionalAttributes = new Map([["class", allowAttribute], ["id", allowAttribute], ["is", (node,value)=>value === "action-link" || value === ""], ["role", (node,value)=>value === "link"], ["src", (node,value)=>node.tagName === "IMG" && value.startsWith("chrome://")], ["tabindex", allowAttribute]]);
...
<!--_html_template_end_-->`,
    ...
    attached() {
        ...
        this.watch("selectedItems_", (({selection: {items: items}})=>items));
        this.updateFromStore();
    ...
    onDisplayedIdsChanged_: async function(newValue, oldValue) {
        ...
        if (this.matches(":focus-within")) {
            if (this.selectedItems_.size > 0) {
                ...
                selectIndex = this.displayedList_.findIndex((({id: id})=>selectedId === id))
            }
...
export {BrowserProxy, Command, CommandManager, DialogFocusManager, DragInfo, DropPosition, HIDE_FOCUS_RING_ATTRIBUTE, IncognitoAvailability, LOCAL_STORAGE_FOLDER_STATE_KEY, LOCAL_STORAGE_TREE_WIDTH_KEY, MenuSource, ROOT_NODE_ID, Store$1 as Store, StoreClient$1 as StoreClient, canEditNode, canReorderChildren, changeFolderOpen, clearSearch, createBookmark, createEmptyState, deselectItems, editBookmark, getDescendants, getDisplayedList, isShowingSearch, moveBookmark, normalizeNodes, reduceAction, removeBookmark, removeIdsFromObject, removeIdsFromSet, reorderChildren, selectFolder, selectItem, setSearchResults, setSearchTerm, updateAnchor, updateFolderOpenState, updateNodes, updateSelectedFolder, updateSelection};
